...
def pipeline_upload_metrics(file_type, stock_product_agg, mtd_product_metrics,
    ...
    def add_metric(sku, label, metric_key, value, month, year):
        ...
        ma_Product is intentionally not populated. Distributor + Item Code + metric +
        period is the unique import identity. Multiple item codes can map to the same
        Product without overwriting each other's values.
        """
        if metric_key not in METRICS or value == 0:
            return
        distributor_sku = str(sku or label or "").strip()
        item_code = str(label or distributor_sku or "").strip()
        if not item_code:
            return
        if not distributor_sku:
            distributor_sku = item_code
        safe_label = "".join(
            ch for ch in item_code
    ...
    # DEDUP: Distributor + Item Code + Metric + Month + Year.
    # Do not use Product GUID: multiple distributor SKUs can map to one Product.
    for rec in records:
        metric_int = rec.pop("_metric_int")
        month = rec["ma_month"]
        year = rec["ma_year"]
        item_code = str(rec.get("ma_itemcode", "")).strip()
        escaped_item_code = item_code.replace("'", "''")

        filter_str = (f"ma_itemcode eq '{escaped_item_code}'"
                      f" and ma_metric eq {metric_int}"
                      f" and ma_month eq {month}"
                      f" and ma_year eq {year}"
                      f" and _ma_distributor_value eq {dist_guid}")
        check_url = f"{API_URL}/{pd_set}"
        ...
        # Never create when the lookup fails, because that could create duplicates.
        if check_resp.status_code != 200:
            results["metrics_failed"] += 1
            results["errors"].append(
                f"METRIC LOOKUP {item_code}: {check_resp.status_code} "
                f"{check_resp.text[:300]}"
            )
            continue
        ...
        if existing:
            ...
            if rec_id:
                ...
                patch_payload = {
                    ...
                    "ma_itemcode": clean.get("ma_itemcode", ""),
                }
